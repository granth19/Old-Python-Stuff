...
class Circle(Oval):
    ...
    def overlap(self, s):
        r1 = self.getRadius()
        r2 = s.getRadius()
        if self.getCenter().getDistance(s.getCenter()) <= (r1 + r2):
            return True
        else:
            return False
...
class RightTriangle(Polygon):
    def __init__(self,p1,x,y):
        ...
        xLoc= p1.getX()
        yLoc=p1.getY()
        self.hypotenuse = math.sqrt((x * x) + (y * y))
        myPoints.append(Point(xLoc+x,yLoc))
        myPoints.append(Point(xLoc,yLoc+y))
        Polygon.__init__(self,myPoints)

class IsoscelesRightTriangle(RightTriangle):
    def __init__(self,p1,side):
        RightTriangle.__init__(self,p1,side,side)

class Trapeziod(Polygon):
    def __init__(self,p1,b1=50,b2=100,height=25):
        someChords=[p1]
        xLoc = p1.getX()
        yLoc = p1.getY()
        p2 = xLoc+b2
        di = (b2-b1)/2
        corner=(yLoc-height)
        someChords.append(Point((xLoc+di),corner))
        someChords.append(Point((xLoc+di)+b1,corner))
        someChords.append(Point(p2, yLoc))
        Polygon.__init__(self,someChords)
...
def test():
    ...
    t.draw(win)
    t2 = Text(Point(5,4), "Let's see what happens")
    t2.draw(win)
    p = Polygon(Point(1, 1), Point(5, 3), Point(2, 7))
    p.draw(win)
    p2= Polygon(Point(2,2), Point(3,2), Point(3,3), Point(2,3))
    p2.draw(win)
    e = Entry(Point(5, 6), 20)
    e.draw(win)
    win.getMouse()
    p.setFill("red")
    p.setOutline("blue")

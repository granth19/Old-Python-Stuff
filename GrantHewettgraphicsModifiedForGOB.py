...
class Point(GraphicsObject):
    ...
    def getDistance(self, second):
        a = self.getX() - second.getX()
        b = self.getY() - second.getY()
        return math.sqrt(a*a + b*b)

    # method to return x and y values to move 1 unit
    # from Point f in the direction of Point other
    def getUnitVectorToward(self, other):
        a = other.getX() - self.getX()
        b = other.getY() - self.getY()
        c = math.sqrt(a * a + b * b)
        dx = -a / c
        dy = -b / c
        return dx, dy
...
class Circle(Oval):
    ...
    def __str__(self):
        return "Circle of area = " + str(3.14*self.radius*self.radius)
    ...
    def overlaps(self, otherCircle):
        return self.getRadius() + otherCircle.getRadius() >= self.getCenter().getDistance(otherCircle.getCenter())
...
def test():
    ...
    t.draw(win)
    t2 = Text(Point(4,2),"Some New Text")
    t2.draw(win)
    T3 = Text(Point(7,7), "Total Newb")
    T3.draw(win)
